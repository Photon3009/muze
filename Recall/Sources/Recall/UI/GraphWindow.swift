...
struct GraphView: View {
    ...
    private let bg = Color(red: 0.11, green: 0.105, blue: 0.10)
    private let leaf = Color(red: 0.85, green: 0.45, blue: 0.55) // rose dots
    private let nodeFill = Color(red: 0.16, green: 0.155, blue: 0.15)
            ...
            GeometryReader { geo in
                canvas(size: geo.size)
                    ...
                    .onTapGesture { location in
                        sim.selected = sim.node(at: location, transform: transform, in: geo.size)
                        if case .memory(let m) = sim.selected?.kind { sim.ensureContent(for: m, engine: engine) }
                    }
                ...
                // Floating preview card (image-style: near the node, left side)
                if let viz = sim.selected ?? sim.hovered,
    ...
    private var toolbar: some View {
        HStack(spacing: 12) {
            Picker("", selection: $sim.scope) {
                ForEach(GraphService.Scope.allCases, id: \.self) { s in
            ...
            Text("\(memories) memories · drag to pan · pinch to zoom")
    ...
    private var dragGesture: some Gesture {
        DragGesture(minimumDistance: 2)
            .onChanged { v in
                if dragStart == nil { dragStart = transform.pan }
