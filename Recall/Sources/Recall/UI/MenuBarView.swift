    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                serviceDot("ollama", up: engine.ollamaUp)
                if engine.pendingCount > 0 {
                    Text("\(engine.pendingCount) queued").font(.caption2).foregroundStyle(.secondary)
                }
            }

            Button("Open Recall") { MainWindowController.shared.show(engine: engine) }
            ...
            Button("Timeline…") { MainWindowController.shared.show(engine: engine, tab: .timeline) }
            ...
            Divider()
            Button("Quit Recall") { NSApp.terminate(nil) }
        }
        .padding(12)
        .frame(width: 260)
    }
