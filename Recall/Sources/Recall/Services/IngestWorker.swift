...
actor IngestWorker {
    ...
    private var ollama: OllamaClient {
        OllamaClient(baseURL: settings.ollamaURLValue, model: settings.ollamaModel)
    }
    ...
    private func ingestSession(_ s: Store.PendingSession) async {
        ...
        if await ollama.isUp(), !novel.isEmpty,
           let e = await EnrichmentService(ollama: ollama).enrichSession(digest: digest) {
            enrichment = e
        } else {
    ...
    private func drainOnce() async -> Bool {
        ...
        let ollamaUp = await ollama.isUp()
        let enricher = EnrichmentService(ollama: ollama)
        ...
        for item in batch {
            ...
            let enrichment: Enrichment
            if ollamaUp, !frame.ocrText.isEmpty,
